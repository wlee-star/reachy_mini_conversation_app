...
| `ask_hermes` | Forward advanced delegated tasks to the Hermes Gateway, such as other buses/trains (not live Route 311), research, multi-step household tasks, or reef report/trend/analysis from the Reefy `reef_thread.jsonl` cache. Direct `apex__*` / `home_assistant__*` MCP tools are not registered while this tool is on. | Set `HERMES_GATEWAY_URL` and `HERMES_API_KEY`. |
