...
class HuggingFaceRealtimeHandler(ConversationHandler):
    ...
    async def _handle_tool_result(self, completed_tool: ToolNotification) -> None:
        ...
        _agent_debug_log(
            "pre-fix",
            "D",
            "huggingface_realtime.py:_handle_tool_result",
            "background tool notification received",
            {
                "tool_name": completed_tool.tool_name,
                "call_id": completed_tool.id,
                "status": completed_tool.status.value,
                "has_result": completed_tool.result is not None,
                "has_error": completed_tool.error is not None,
                "connection_open": self.connection is not None,
            },
        )
        # endregion
        ...
        try:
            ...
            announce_stale_result = (
                send_result_to_model and result_is_stale and completed_tool.tool_name == "ask_hermes"
            )
            if send_result_to_model and result_is_stale and not announce_stale_result:
                logger.warning(
            ...
            elif announce_stale_result:
                logger.info(
                    "Announcing completed Hermes result from an earlier turn (id=%s)",
                    completed_tool.id,
                )
            if send_result_to_model and isinstance(completed_tool.id, str):
                ...
                _agent_debug_log(
                    ...
                    "D",
                    "huggingface_realtime.py:_handle_tool_result",
                    "tool result submitted to realtime conversation",
                    {
                        "tool_name": completed_tool.tool_name,
                        "call_id": completed_tool.id,
                        "result_keys": sorted(tool_result_for_model)
                        if isinstance(tool_result_for_model, dict)
                        else [],
                        "has_error": isinstance(tool_result_for_model, dict) and "error" in tool_result_for_model,
                    },
                )
                # endregion
            ...
            if model_result_submitted and announce_stale_result:
                await self._safe_response_create(reason="tool_result:ask_hermes", **_TOOL_FOLLOWUP_CREATE_KWARGS)
                return
    ...
    async def _run_realtime_session(self) -> None:
        ...
        _agent_debug_log(
            "pre-fix",
            "A,B",
            "huggingface_realtime.py:_run_realtime_session",
            "realtime session preparing tool specs",
            {
                "connection_mode": get_hf_connection_selection().mode,
                "direct_ws_configured": bool(get_hf_direct_ws_url()),
                "tool_count": len(tool_specs),
                "tool_names": [tool["name"] for tool in tool_specs],
                "instance_path_set": self.instance_path is not None,
                "has_home_assistant": any(tool["name"] == "home_assistant" for tool in tool_specs),
                "has_move_head": any(tool["name"] == "move_head" for tool in tool_specs),
            },
        )
        # endregion
        logger.info(
        ...
        async with self.client.realtime.connect(**connect_kwargs) as conn:
            try:
                ...
                _agent_debug_log(
                    ...
                    "A",
                    "huggingface_realtime.py:_run_realtime_session",
                    "realtime session.update completed with tools",
                    {
                        "connection_mode": get_hf_connection_selection().mode,
                        "tool_count": len(tool_specs),
                        "tool_choice": session_config.get("tool_choice"),
                    },
                )
                # endregion
                logger.info(
                ...
                async for event in self.connection:
                    ...
                    if event.type == "response.function_call_arguments.done":
                        ...
                        _agent_debug_log(
                            "pre-fix",
                            "C",
                            "huggingface_realtime.py:response.function_call_arguments.done",
                            "realtime function-call event received",
                            {
                                "tool_name": tool_name,
                                "call_id": call_id,
                                "args_is_json_string": isinstance(args_json_str, str),
                                "registered": isinstance(tool_name, str) and tool_name in registry,
