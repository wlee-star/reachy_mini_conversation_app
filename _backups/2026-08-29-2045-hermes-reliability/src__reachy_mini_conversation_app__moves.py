...
from reachy_mini_conversation_app.tools.core_tools import _agent_debug_log
...
class MovementManager:
    ...
    def _handle_command(self, command: str, payload: Any, current_time: float) -> None:
        ...
        if command in {"queue_move", "set_head_tracking", "set_speaking", "set_listening"}:
            # region agent log
            _agent_debug_log(
                "pre-fix",
                "E",
                "moves.py:MovementManager._handle_command",
                "movement command handled",
                {
                    "command": command,
                    "payload_type": type(payload).__name__,
                    "payload_bool": payload if isinstance(payload, bool) else None,
                    "queue_size_before": len(self.move_queue),
                    "head_tracking": self._head_tracking,
