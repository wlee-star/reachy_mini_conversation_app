...
from reachy_mini_conversation_app.tools.core_tools import Tool, ToolDependencies, _agent_debug_log
...
class HomeAssistant(Tool):
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        ...
        scene_id = _require_string(kwargs.get("scene_id"))
        # region agent log
        _agent_debug_log(
            "pre-fix",
            "E",
            "tools/home_assistant.py:HomeAssistant.__call__",
            "home assistant tool entered",
            {
                "action": action,
                "has_entity_id": entity_id is not None,
                "entity_domain": entity_id.split(".", 1)[0] if entity_id and "." in entity_id else None,
                "has_scene_id": scene_id is not None,
                "ha_url_configured": bool((config.HA_URL or "").strip()),
                "ha_token_configured": bool((config.HA_TOKEN or "").strip()),
            },
        )
        # endregion
        valid_actions = {
