...
from reachy_mini_conversation_app.huggingface_realtime import RealtimeSessionSlotsBusy, HuggingFaceRealtimeHandler
...
@pytest.mark.asyncio
async def test_stale_hermes_result_is_announced(monkeypatch: Any) -> None:
    """Long-running Hermes research should still be delivered after a newer turn."""
    handler = HuggingFaceRealtimeHandler(ToolDependencies(reachy_mini=MagicMock(), movement_manager=MagicMock()))
    handler.connection = AsyncMock()
    handler.output_queue = asyncio.Queue()
    monkeypatch.setattr(handler, "_wait_for_response_done_before_tool_result", AsyncMock(return_value=True))
