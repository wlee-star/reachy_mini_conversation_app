...
_BUS_QUERY_RE = re.compile(
    r"\b(?:311|next bus|the bus|bus (?:is |coming|arrival|arriving)|when(?:'s| is) the bus|"
...
async def fetch_live_snapshot() -> LiveBusSnapshot:
    ...
    return LiveBusSnapshot(
        ...
        stale=stale,
        ha_query_latency_s=latency,
        error=None if arrivals else "Bus arrival data unavailable",
        fetched_at=time.time(),
    )
...
class BusMonitorManager:
    ...
    async def resume_from_disk(self) -> dict[str, Any]:
        ...
        snapshot = await fetch_live_snapshot()
        arrival = snapshot.next_arrival
        if snapshot.error or arrival is None:
        ...
        if not same_service(loaded, arrival) and arrival.minutes > DEFAULT_URGENT_MINUTES:
            loaded.service_id = arrival.service_id
            loaded.route = arrival.route
            loaded.destination = arrival.destination
            loaded.latest_live_eta = arrival.minutes
            loaded.preparation_alert_sent = arrival.minutes <= loaded.preparation_threshold
            loaded.urgent_alert_sent = arrival.minutes <= loaded.urgent_threshold
        elif not same_service(loaded, arrival):
            self._monitor = None
            self._persist()
            return {"status": "expired"}
        else:
            ...
            if arrival.minutes <= loaded.urgent_threshold:
                ...
                loaded.preparation_alert_sent = True
        if loaded.urgent_alert_sent and arrival.minutes <= loaded.urgent_threshold:
            self._monitor = None
            self._persist()
            return {"status": "expired"}
        self._monitor = loaded
    ...
    async def _poll_once(self) -> None:
        ...
        arrival = snapshot.next_arrival
        if arrival is None:
            logger.info("Bus monitor completed; no upcoming 311 id=%s", monitor.monitor_id)
            self._monitor = None
            self._persist()
            return
        if not same_service(monitor, arrival):
            if arrival.minutes <= monitor.urgent_threshold:
                logger.info("Monitored 311 departed; next service is already urgent. Completing monitor.")
                self._monitor = None
                self._persist()
                return
            logger.info("Monitored 311 gone; switching to next live service eta=%s", arrival.minutes)
            monitor.service_id = arrival.service_id
            monitor.route = arrival.route
            monitor.destination = arrival.destination
            monitor.preparation_alert_sent = arrival.minutes <= monitor.preparation_threshold
            monitor.urgent_alert_sent = False
