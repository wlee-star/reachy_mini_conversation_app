...
| `monitor_bus` | Query the live Home Assistant Route 311 sensor, then optionally watch that specific service in the background for 10/15-minute, 5-minute, and arrival alerts. Does not add a second bus API. Active watches persist in `bus_monitors.v1.json`. | Same Home Assistant config as `home_assistant`. |
