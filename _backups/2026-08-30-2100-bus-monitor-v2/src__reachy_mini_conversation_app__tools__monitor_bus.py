...
class MonitorBus(Tool):
    ...
    description = (
        ...
        "start or cancel background monitoring. Always call query first for a current 311 time. "
    ...
    parameters_schema = {
        ...
        "properties": {
            "action": {
                "type": "string",
                "enum": ["query", "start", "cancel", "status"],
                "description": "query reads live Home Assistant data; start begins watching; cancel stops; status reports the watch.",
            },
    ...
    async def __call__(self, deps: ToolDependencies, **kwargs: Any) -> dict[str, Any]:
        ...
        if action not in {"query", "start", "cancel", "status"}:
            return {"error": "action must be one of query, start, cancel, status"}
