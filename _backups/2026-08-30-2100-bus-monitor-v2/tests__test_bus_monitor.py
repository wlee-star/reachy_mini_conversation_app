...
def test_evaluate_alerts_do_not_repeat_when_eta_oscillates() -> None:
    ...
    assert evaluate_alerts(monitor, _arrival(4)) is None
    assert evaluate_alerts(monitor, _arrival(0)) == "arrival"
    monitor.arrival_alert_sent = True
    assert evaluate_alerts(monitor, _arrival(0)) is None
...
async def test_start_follows_live_eta_changes_and_alerts_once(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert any("due now" in item for item in alerts)
    ...
    assert sum("due now" in item for item in alerts) == 1
...
def test_urgent_message_is_deterministic() -> None:
    ...
    assert format_arrival_alert(_arrival(0)) == "The 311 is due now. Please leave now if you haven't already."
    assert format_arrival_alert(None) == "The 311 has arrived. Please leave now if you haven't already."
...
async def test_skipping_zero_from_five_minutes_to_next_service(
    ...
    await manager.start()
    await _wait_until_idle(manager)
    assert sum("5 minutes away" in item for item in alerts) == 1
    assert sum("has arrived" in item for item in alerts) == 1
    queried = await manager.query()
    assert queried["minutes"] == 28
    assert manager.monitor_active() is False
