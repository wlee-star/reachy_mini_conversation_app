...
@pytest.mark.asyncio
async def test_monitor_bus_switch_uses_manager(monkeypatch: pytest.MonkeyPatch, _manager: BusMonitorManager) -> None:
    """Switch is a first-class tool action, not a Hermes or LLM countdown."""

    async def _switch() -> dict[str, object]:
        return {
            "status": "monitoring",
            "spoken": "Okay. I've switched the monitor to the 311 arriving at approximately 9:05.",
        }

    monkeypatch.setattr(_manager, "switch", _switch)
    result = await MonitorBus()(_deps(), action="switch")
    assert result["status"] == "monitoring"
    assert "9:05" in result["spoken"]
